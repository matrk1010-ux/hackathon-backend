...
2. 次点プラン：一度にまとめて揃う包括的な選択肢（全巻セットや広い範囲のセット）。
   ...
   **候補に全巻セットや広い範囲のセットが存在する場合は、たとえ所有巻と一部重複しても、必ずこの「まとめ買いプラン」を2つ目として提示すること（1プランだけで済ませない）。**
3. 重複は可能な限り少なくする。不足分を過不足なく満たせる組み合わせがあれば、それを最優先プランにする。
4. 可能なら3つ目に「別の組み合わせ（部分セット中心など）」も提示してよい。
...
- これは表示用の目印（重複の有無をユーザーに示すため）。重複プランも隠さず提示するので、まとめ買いプランを省略しないこと。正確に付けること。
