...
- 生成AI：Google Gemini `gemini-2.5-flash`（説明生成・AIセット会話）
...
- AIセットの出力契約：本文末尾の `<PLANS>[{"title":str,"reason":str,"ids":[int,...],"owned_overlap":bool}]</PLANS>` タグ（最大3つの「買い方プラン」）。**プロンプトを変えるならパーサ（`ai_set.py`）も必ず合わせる。** ids は候補に実在するもののみ採用、プラン内重複は排除。`owned_overlap` はユーザーが既に所有と述べたアイテムを含むプランの印（現状はコード側では除外に使わず、最大3プランをそのまま提示する。「不足分だけ」と「全巻セット等のまとめ買い」を両方見せるため）。
